...
## 2026-07-07 — In-app updater via GitHub Releases (not expo-updates), repo made public
**Why:** chat-to-pdf is sideloaded (EAS APK, no Play Store), so a user on an old APK has no update path. Ported the mechanism from a friend's app (Snehit70/pomo): manual "Check for updates" → GitHub Releases `/latest` → semver compare → download the `.apk` asset → verify against `.apk.sha256` → hand to Android's package installer. Rejected `expo-updates` — it swaps the JS bundle OTA and cannot replace a whole native APK or move someone off an old binary. Required making the repo **public**: the app calls the anonymous GitHub API, which returns 404 for BOTH the release check and the asset download on a private repo (embedding a token in an APK is a leak). Release contract: tag `vX.Y.Z`, exactly one `.apk` + one `.apk.sha256`, every build signed with the SAME EAS keystore (in-place update depends on it — regenerating it strands all installed users). Verified end-to-end on-device v1.0.0→v1.0.1; Expo's built-in FileProvider (`getContentUriAsync`) is accepted by the installer, so no custom FileProvider plugin is needed.

## 2026-07-07 — sha256 download verification is non-blocking (degrades to a warning)
**Why:** A checksum *mismatch* blocks and deletes the file (tamper/corruption guard), but a *failure to compute* the hash degrades to an "install with caution" warning and still allows install — so a device-side hashing limitation can't brick the only update path. This is currently load-bearing: on-device `sha256OfFile` OOMs on the ~60MB base64→bytes round-trip, so verification is effectively inert until switched to chunked/streamed hashing. Chose availability of the update path over hard checksum enforcement given HTTPS already protects transport.
